...
export class Reporter {
	...
	finish(session: SessionLike, terminal: Terminal): Record<string, unknown> {
		...
		return {
			...
			counts: { turns: stats.userMessages, model_turns: this.trace.filter((event) => event.type === "turn_start").length, provider_requests: null, tool_calls: stats.toolCalls, retries: 0, compactions: this.trace.filter((event) => event.type === "compaction_end").length },
