...
export class Reporter {
	...
	finish(session: SessionLike, terminal: Terminal): Record<string, unknown> {
		...
		return {
			...
			counts: { turns: stats.userMessages, provider_requests: null, tool_calls: stats.toolCalls, retries: 0, compactions: this.trace.filter((event) => event.type === "compaction_end").length },
			usage: { input: stats.tokens.input, output: stats.tokens.output, generation: stats.tokens.input + stats.tokens.output, reasoning: null, cache_read: stats.tokens.cacheRead, cache_write: stats.tokens.cacheWrite },
