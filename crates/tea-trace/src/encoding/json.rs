...
pub fn decode_json_line(line: &str) -> Result<TraceEvent, JsonTraceDecodeError> {
    ...
    let object = match value {
        miniserde::json::Value::Object(value) => value,
        _ => return Err(JsonTraceDecodeError::new("trace record is not an object")),
    };
    let schema = take_u64(&object, "schema_version")?;
    if schema != u64::from(crate::TRACE_SCHEMA_VERSION) {
...
pub fn decode_jsonl(input: &str) -> Result<Vec<TraceEvent>, JsonTraceDecodeError> {
    if input.is_empty() {
        return Err(JsonTraceDecodeError::new("empty trace"));
    }
    ...
    if !matches!(events.first(), Some(TraceEvent::EpisodeHeader(_)))
        ...
        || events
            .iter()
            .skip(1)
            .any(|event| matches!(event, TraceEvent::EpisodeHeader(_)))
        || events
            .iter()
            .take(events.len().saturating_sub(1))
            .any(TraceEvent::is_terminal)
    {
...
fn take<'a>(o: &'a Obj, n: &str) -> Result<&'a miniserde::json::Value, JsonTraceDecodeError> {
    o.get(n)
        .ok_or_else(|| JsonTraceDecodeError::new(format!("missing field {n}")))
}
fn take_str(o: &Obj, n: &str) -> Result<String, JsonTraceDecodeError> {
    match take(o, n)? {
        miniserde::json::Value::String(v) => Ok(v.clone()),
        _ => Err(JsonTraceDecodeError::new(format!(
            "field {n} has wrong type"
        ))),
    }
}
fn take_opt_str(o: &Obj, n: &str) -> Result<Option<String>, JsonTraceDecodeError> {
    match o.get(n) {
        None | Some(miniserde::json::Value::Null) => Ok(None),
        Some(miniserde::json::Value::String(v)) => Ok(Some(v.clone())),
        Some(_) => Err(JsonTraceDecodeError::new(format!(
            "field {n} has wrong type"
        ))),
    }
}
fn take_u64(o: &Obj, n: &str) -> Result<u64, JsonTraceDecodeError> {
    match take(o, n)? {
        miniserde::json::Value::Number(miniserde::json::Number::U64(v)) => Ok(*v),
        _ => Err(JsonTraceDecodeError::new(format!(
            "field {n} has wrong type"
        ))),
    }
}
fn take_opt_u64(o: &Obj, n: &str) -> Result<Option<u64>, JsonTraceDecodeError> {
    match o.get(n) {
        None | Some(miniserde::json::Value::Null) => Ok(None),
        Some(miniserde::json::Value::Number(miniserde::json::Number::U64(v))) => Ok(Some(*v)),
        Some(_) => Err(JsonTraceDecodeError::new(format!(
            "field {n} has wrong type"
        ))),
    }
}
fn take_opt_bool(o: &Obj, n: &str) -> Result<Option<bool>, JsonTraceDecodeError> {
    match o.get(n) {
        None | Some(miniserde::json::Value::Null) => Ok(None),
        Some(miniserde::json::Value::Bool(v)) => Ok(Some(*v)),
        Some(_) => Err(JsonTraceDecodeError::new(format!(
            "field {n} has wrong type"
        ))),
    }
}
fn take_map_str(o: &Obj, n: &str) -> Result<BTreeMap<String, String>, JsonTraceDecodeError> {
    match take(o, n)? {
        miniserde::json::Value::Object(m) => m
            .iter()
            .map(|(k, v)| match v {
                miniserde::json::Value::String(s) => Ok((k.clone(), s.clone())),
...
fn take_strings(o: &Obj, n: &str) -> Result<Vec<String>, JsonTraceDecodeError> {
    match take(o, n)? {
        miniserde::json::Value::Array(a) => a
            .iter()
            .map(|v| match v {
                miniserde::json::Value::String(s) => Ok(s.clone()),
                _ => Err(JsonTraceDecodeError::new("array value has wrong type")),
            })
            .collect(),
        _ => Err(JsonTraceDecodeError::new(format!(
            "field {n} has wrong type"
...
fn parse_header(o: &Obj) -> Result<TraceEvent, JsonTraceDecodeError> {
    let mut h = EpisodeHeader::new(take_str(o, "episode_id")?);
    h.metadata = take_map_str(o, "metadata")?;
    h.started_at_ms = take_opt_u64(o, "started_at_ms")?;
    h.provenance = parse_provenance(o)?;
    Ok(h.into())
}
fn parse_provenance(o: &Obj) -> Result<Option<TraceProvenance>, JsonTraceDecodeError> {
    match take(o, "provenance")? {
        miniserde::json::Value::Null => Ok(None),
        miniserde::json::Value::Object(p) => Ok(Some(TraceProvenance {
            session_id: take_opt_str(p, "session_id")?,
            lane_id: take_opt_str(p, "lane_id")?,
            agent_id: take_opt_str(p, "agent_id")?,
            operation_id: take_opt_str(p, "operation_id")?,
            epoch_id: take_opt_str(p, "epoch_id")?,
            core_run_id: take_opt_str(p, "core_run_id")?,
            harness_snapshot_id: take_opt_str(p, "harness_snapshot_id")?,
            harness_revision_id: take_opt_str(p, "harness_revision_id")?,
            model_harness_profile_id: take_opt_str(p, "model_harness_profile_id")?,
            experiment_id: take_opt_str(p, "experiment_id")?,
        })),
        _ => Err(JsonTraceDecodeError::new("field provenance has wrong type")),
    }
}
fn parse_turn(o: &Obj) -> Result<TraceEvent, JsonTraceDecodeError> {
    let mut t = Turn::new(
        u32::try_from(take_u64(o, "index")?)
            .map_err(|_| JsonTraceDecodeError::new("index out of range"))?,
        take_str(o, "input")?,
    );
    t.output = take_opt_str(o, "output")?;
    t.stop_reason = take_opt_str(o, "stop_reason")?;
    t.cache_evidence = parse_cache(o)?;
    Ok(t.into())
}
fn parse_tool(o: &Obj) -> Result<TraceEvent, JsonTraceDecodeError> {
    let mut t = Tool::new(
        u32::try_from(take_u64(o, "turn_index")?)
            .map_err(|_| JsonTraceDecodeError::new("turn_index out of range"))?,
        take_str(o, "call_id")?,
        take_str(o, "name")?,
        take_str(o, "input")?,
    );
    t.output = take_opt_str(o, "output")?;
    t.error = take_opt_str(o, "error")?;
    Ok(t.into())
}
fn parse_end(o: &Obj) -> Result<TraceEvent, JsonTraceDecodeError> {
    let reason = match take_str(o, "reason")?.as_str() {
        "completed" => EndReason::Completed,
        "cancelled" => EndReason::Cancelled,
        "failed" => EndReason::Failed,
        "aborted" => EndReason::Aborted,
        v => EndReason::Other(v.to_owned()),
    };
    Ok(EpisodeEnd {
        reason,
        error: take_opt_str(o, "error")?,
        finished_at_ms: take_opt_u64(o, "finished_at_ms")?,
    }
    .into())
}
fn parse_cache(o: &Obj) -> Result<Option<CacheEvidence>, JsonTraceDecodeError> {
    let v = take(o, "cache_evidence")?;
    let miniserde::json::Value::Object(p) = v else {
        if matches!(v, miniserde::json::Value::Null) {
            return Ok(None);
        }
        return Err(JsonTraceDecodeError::new(
...
fn parse_compaction(o: &Obj) -> Result<TraceEvent, JsonTraceDecodeError> {
    let stage = match take_str(o, "stage")?.as_str() {
        "started" => CompactionStage::Started,
        "source_selected" => CompactionStage::SourceSelected,
        "request_prepared" => CompactionStage::RequestPrepared,
        "provider_usage_observed" => CompactionStage::ProviderUsageObserved,
        "replacement_proposed" => CompactionStage::ReplacementProposed,
        "terminal" => CompactionStage::Terminal,
        "post_compaction_request_observed" => CompactionStage::PostCompactionRequestObserved,
        _ => return Err(JsonTraceDecodeError::new("unknown compaction stage")),
    };
    let mut c = Compaction::new(take_str(o, "compaction_id")?, stage);
    macro_rules! s {
        ($f:ident) => {
            c.$f = take_opt_str(o, stringify!($f))?;
        };
    }
    macro_rules! n {
        ($f:ident) => {
            c.$f = take_opt_u64(o, stringify!($f))?
                .map(|v| usize::try_from(v))
                .transpose()
                .map_err(|_| JsonTraceDecodeError::new("number out of range"))?;
        };
    }
    macro_rules! u {
        ($f:ident) => {
    ...
    macro_rules! b {
        ($f:ident) => {
            c.$f = take_opt_bool(o, stringify!($f))?;
        };
    }
    s!(trigger);
    s!(reason);
    s!(phase);
    s!(strategy_id);
    u!(strategy_schema_version);
    s!(request_layout);
    q!(prompt_fingerprint);
    q!(source_history_revision);
    u!(attempt);
    u!(automatic_ordinal);
    u!(overflow_retry_ordinal);
    b!(retry_provider_request);
    n!(source_message_count);
    n!(source_message_bytes);
    n!(retained_message_count);
    n!(retained_suffix_bytes);
    n!(tool_result_bytes);
    n!(compactor_context_bytes);
    n!(compactor_tool_count);
    b!(tools_execution_prohibited);
    b!(source_is_active_context_prefix);
    n!(replacement_message_count);
    n!(replacement_bytes);
    q!(estimated_context_tokens_after);
    q!(headroom_tokens);
    b!(structural_validation_passed);
    b!(retained_suffix_exact);
    b!(source_generation_matches);
    q!(provider_input_tokens);
    q!(provider_output_tokens);
    q!(provider_cache_read_tokens);
    q!(provider_cache_write_tokens);
    n!(serialized_request_bytes);
    q!(cache_domain_fingerprint);
    s!(terminal_outcome);
    u!(post_compaction_turn_index);
    Ok(c.into())
}
