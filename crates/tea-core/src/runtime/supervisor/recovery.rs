...
pub(super) fn committed_run_outcome(
    ...
    let messages = snapshot
        ...
        .filter(|entry| {
            &entry.lane_id == lane_id
                && entry.header.seq >= epoch_sequence
                && matches!(
                    &entry.body,
        ...
        .collect::<Vec<_>>();
    if matches!(
        messages.last().map(|entry| &entry.body),
        Some(SessionEntry::ToolResult(_))
    ) {
        let Some(assistant_index) = messages
            .iter()
            .rposition(|entry| matches!(&entry.body, SessionEntry::AssistantMessage(_)))
        else {
            return Ok(None);
        };
        let SessionEntry::AssistantMessage(assistant) = &messages[assistant_index].body else {
            unreachable!()
        };
    ...
    let Some(SessionEntry::AssistantMessage(assistant)) = messages.last().map(|entry| &entry.body)
    else {
        return Ok(None);
    };
    ...
    Ok(
            ...
            .transpose()?
        {
            ...
            Some(StopReason::Error) => Some(OperationOutcome::Failed {
                code: "model_error".into(),
            }),
            Some(StopReason::Aborted) => Some(OperationOutcome::Failed {
                code: "model_aborted".into(),
            }),
            _ => None,
        },
    )
...
pub fn inspect_recovery(snapshot: &SessionSnapshot) -> Result<RecoveryReport, HarnessError> {
    ...
    for lane_id in lane_ids {
        ...
        for (entry_index, entry) in snapshot
            .entries()
            .iter()
            .enumerate()
            .filter(|(_, entry)| entry.lane_id == lane_id)
        {
            ...
            for (index, call) in assistant.tool_calls.iter().enumerate() {
                ...
                if !invoked && !settled {
                    effects.push(InterruptedEffect::ToolNotInvoked {
                        assistant_entry_id: entry.header.id.clone(),
        ...
        if reduction.lane_state.active_operation.is_some() || !effects.is_empty() {
            lanes.push(LaneRecoveryReport {
                lane_id,
                operation_id: reduction.lane_state.active_operation,
                next: reduction.recovery_plan,
                effects,
            });
        }
    }
    ...
    S: SessionWriter + Send + 'static,
{
    ...
    pub(super) fn ensure_recovery_permitted(&self, lane_id: &LaneId) -> Result<(), HarnessError> {
        ...
        let operations = snapshot
            ...
            .collect::<BTreeSet<_>>();
        for stored in snapshot.records() {
            let LaneRecord::ToolStarted(tool) = &stored.record else {
                continue;
            };
    ...
    pub fn reconcile_tool_result(
        ...
        if evidence.trim().is_empty() || evidence.len() > 16 * 1024 {
            return Err(HarnessError::invalid_state(
                "tool reconciliation requires non-empty evidence within 16384 bytes",
            ));
        }
        let lane = self.lane(&lane_id)?;
        ...
        if !owns_operation || result.tool_call_id.as_str() != started.tool_call_id {
            return Err(HarnessError::invalid_state(
                "tool reconciliation does not match its owning lane and invocation",
            ));
        }
        if snapshot
            .entries()
            .iter()
            .any(|entry| entry.header.id == result_entry_id)
        {
            return Err(HarnessError::invalid_state(
                "a committed tool outcome cannot be reconciled again",
            ));
        }
        let configuration = self.configuration_for_revision(&lane, &started.harness_revision_id)?;
        let mut retained_result = result;
        retained_result.details = Some(SerializedJson::new(
            JsonValue::object([
                ...
                (
                    ...
                    retained_result
                        .details
                        .as_ref()
                        .map(|details| JsonValue::String(details.as_str().into()))
        ...
        if operation_closed {
            items.extend(self.uninvoked_sibling_results(
                &snapshot,
                &lane_id,
                started,
                &configuration,
    ...
    fn uninvoked_sibling_results(
        ...
        let assistant_index = snapshot
            .entries()
            .iter()
            .position(|entry| entry.header.id == started.assistant_entry_id)
            .ok_or_else(|| {
                HarnessError::invalid_state("reconciliation source assistant is missing")
            })?;
        let SessionEntry::AssistantMessage(assistant) = &snapshot.entries()[assistant_index].body
        else {
            return Err(HarnessError::invalid_state(
                "reconciliation source is not an assistant",
            ));
        };
        ...
        for (index, call) in assistant.tool_calls.iter().enumerate() {
            ...
            let result = AgentToolResult {
                ...
                details: Some(SerializedJson::new(
                    r#"{"invoked":false,"outcome_known":true}"#,
                )),
                usage: None,
            ...
            let entry = tool_result_entry(&result, &result, &call.name, retained)
                .map_err(|error| HarnessError::invalid_state(error.to_string()))?;
            let result_id = EntryId::new(durable_identifier(
                "entry-tool-result",
                [started.assistant_entry_id.as_str(), &index.to_string()],
            ))
            .map_err(|error| HarnessError::invalid_state(error.to_string()))?;
    ...
    pub(super) fn interrupt_provider_attempt(
        ...
        let admitted = self.snapshot()?.facts().iter().any(|stored| {
            matches!(&stored.fact,
            ...
            .append_record(LaneRecord::ProviderRequestSettled(
                ProviderRequestSettledRecord {
                    ...
                    outcome: JsonValue::object([
                        ...
                        (
                            "continuation",
                            JsonValue::String("explicit_new_attempt".into()),
                        ),
                    ]),
                    provider_error: None,
                    usage: None,
