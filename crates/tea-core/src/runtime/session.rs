...
use crate::harness::{
    ...
    SELF_EXTENSION_MODE_METADATA_KEY, SelfExtensionMode, inspect_tool_schema_deviation,
};
use crate::runtime::{
...
pub struct SessionRuntime<S> {
    ...
    /// Host-owned provider transport and trusted executable capabilities.
    /// Resolved harness revisions never own this authority.
...
impl<S> std::fmt::Debug for SessionRuntime<S> {
    ...
    S: SessionWriter + Send + 'static,
{
    ...
    pub fn new_with_artifact_store(
        ...
        let runtime_services = manager.runtime_services();
        validate_reserved_host_tool_names(&runtime_services, &resolved)?;
        Ok(Self {
    ...
    /// immutable catalog.
    ///
    /// The caller supplies the same trusted runtime services, capability catalog,
    ...
    fn epoch_configuration(&self, epoch_id: &EpochId) -> Result<ResolvedHarness, HarnessError> {
        let snapshot = self.snapshot()?;
        let started = snapshot
            .records()
            .iter()
...
impl SessionRuntime<tea_session::JsonlSession> {
