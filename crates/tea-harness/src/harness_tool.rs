    ...
    S: SessionWriter + Send + 'static,
{
    ...
    fn require_mutation_authorization(&self) -> Result<(), String> {
        ...
        if mode == crate::SelfExtensionMode::Off {
    ...
    fn schedule_candidate(
        &self,
        call: &ToolCall,
        candidate: crate::HarnessCandidateV1,
        operation: &str,
    ) -> Result<ControlResponse, String> {
        ...
        if !candidate.validation.accepted || candidate.validation.is_noop {
            let (stage, code, diagnostic) = if candidate.validation.is_noop {
                ...
                .draft
                .changed_surfaces
                .contains(&crate::HarnessSurface::CapabilityBindings)
            {
                (
        ...
        let revision_entry_id = EntryId::new(crate::supervisor::durable_identifier(
            ...
            && operation.original_input.iter().any(|entry| {
                matches!(
                    ...
                    tea_session::SessionEntry::UserMessage(message)
                        if message.metadata.get(crate::AUTHORING_AUTHORIZATION_METADATA_KEY)
...
fn actor_name(actor: crate::HarnessActor) -> &'static str {
    match actor {
        crate::HarnessActor::Host => "host",
        crate::HarnessActor::Operator => "operator",
        crate::HarnessActor::Model => "model",
    }
}

fn revision_reason(reason: &crate::HarnessRevisionReason) -> &'static str {
    match reason {
        crate::HarnessRevisionReason::Initial => "initial",
        crate::HarnessRevisionReason::CandidateActivation => "candidate_activation",
        crate::HarnessRevisionReason::GlobalRebase => "global_rebase",
        crate::HarnessRevisionReason::Rollback => "rollback",
    }
}
