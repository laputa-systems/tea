...
impl PluginLifecycleRegistry {
    ...
    pub(crate) fn from_loaded(
        ...
        let policies = policies
            .iter()
            .map(|loaded| {
                for local_id in loaded.policy.resume_hook_ids().map_err(policy_error)? {
                    let _ = stable_hook_id(&loaded.plugin.plugin_id, &local_id)?;
                }
                Ok(PluginLifecyclePolicy {
    ...
    pub(crate) fn before_operation(
        ...
        self.collect_state("before_operation", |policy| {
            policy.before_operation_resume_data()
        })
    }
    ...
    pub(crate) fn before_resume(
        ...
        for registered in &self.policies {
            ...
            for local_id in registered.policy.resume_hook_ids().map_err(policy_error)? {
    ...
    fn collect_state(
        &self,
        phase: &str,
        invoke: impl Fn(&LuaPolicy) -> Result<BTreeMap<String, JsonValue>, tea_luau::PolicyError>,
    ) -> Result<BTreeMap<StableHookId, JsonValue>, HarnessError> {
        let mut durable = BTreeMap::new();
        for registered in &self.policies {
            let values = invoke(&registered.policy).map_err(policy_error)?;
...
fn policy_error(error: tea_luau::PolicyError) -> HarnessError {
    HarnessError::invalid_state(format!("Luau lifecycle policy failed: {error}"))
}
