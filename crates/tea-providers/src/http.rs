...
use http::{HeaderValue, Method as HttpMethod, Request as HttpRequest, Uri, header::HeaderName};
...
#[cfg(test)]
mod tests {
    use super::{Request, StreamEvent, send, stream};
    use crate::scheduler::CancellationToken;
    use std::io::Write;
    ...
    fn streaming_transport_yields_a_body_chunk_before_the_response_settles() {
        ...
        loop {
            match smol::block_on(std::future::poll_fn(|context| response.poll_next(context))) {
                StreamEvent::Chunk(bytes) => suffix.extend(bytes),
                StreamEvent::End => break,
                unexpected => panic!("stream suffix has unexpected event: {unexpected:?}"),
            }
        }
