...
use tea_core::effect::EffectGateError;
...
use tea_providers::commandcode::{CommandCodeConfig, CommandCodeHostContext, CommandCodeProvider};
use tea_providers::openai::OpenAiContextHook;
use tea_providers::openrouter::{OpenRouterConfig, OpenRouterCostReport, OpenRouterProvider};
...
fn terminal_status(result: &Result<(), CoreError>) -> &'static str {
    match result {
        Ok(()) => "completed",
        Err(CoreError::Cancelled) => "cancelled",
        Err(CoreError::ModelAborted { .. }) => "aborted",
        Err(_) => "failed",
    }
}
...
fn terminal_code(result: &Result<(), CoreError>) -> Option<&'static str> {
    match result {
        Ok(()) => None,
        Err(CoreError::Cancelled) => Some("cancelled"),
        Err(CoreError::ModelAborted { .. }) => Some("model_aborted"),
        Err(CoreError::ModelError { .. }) => Some("model_error"),
        Err(CoreError::ModelProvider { .. }) => Some("model_provider"),
        Err(CoreError::UnsupportedModelStream { .. }) => Some("unsupported_model_stream"),
        Err(CoreError::Hook(_)) => Some("hook"),
        Err(CoreError::EffectGate(_)) => Some("effect_gate"),
        Err(CoreError::MissingModelProvider) => Some("missing_model_provider"),
        Err(CoreError::ActiveRun { .. }) => Some("active_run"),
        Err(CoreError::InvalidTransition(_)) => Some("invalid_transition"),
        Err(CoreError::RunFinished { .. }) => Some("run_finished"),
        Err(CoreError::Compaction(_)) => Some("compaction"),
        Err(CoreError::MissingCompactor) => Some("missing_compactor"),
        Err(CoreError::AutomaticCompactionUnavailable { .. }) => {
            Some("automatic_compaction_unavailable")
        }
        Err(CoreError::AutomaticCompaction { .. }) => Some("automatic_compaction"),
        Err(CoreError::InvalidAutomaticCompactionPolicy { .. }) => {
            Some("invalid_automatic_compaction_policy")
        }
        Err(CoreError::InvalidToolResultProjectionPolicy { .. }) => {
            Some("invalid_tool_result_projection_policy")
        }
        Err(CoreError::ToolCircuitBreaker { .. }) => Some("tool_circuit_breaker"),
    }
}
...
fn main() -> Result<(), String> {
    ...
    let profile = PiDefaultCodingProfile::pinned_default()
        .map_err(|error| format!("cannot load pinned default profile: {error}"))?;
    let tool_registry = default_tools.registry();
    profile
        .validate_registry(&tool_registry)
    ...
    let agent = Agent::builder()
        ...
        .system_prompt(profile.system_prompt_for_workspace(default_tools.workspace().as_path()))
        .tools(tool_registry)
        .build();
    let run = agent
        .start_prompt(prompt)
...
mod tests {
    ...
    fn terminal_code_redacts_effect_gate_failures() {
        let error = CoreError::EffectGate(EffectGateError::new(
